...
#get pcs
#system(paste0("plink --bfile /well/todd/users/jinshaw/aad/geno_unrel_uk --indep-pairwise 1000 50 0.2 --out /well/todd/users/jinshaw/aad/under_7/pruned_uk --allow-no-sex"))
#system(paste0("plink --bfile /well/todd/users/jinshaw/aad/geno_unrel_uk --exclude /well/todd/users/jinshaw/aad/under_7/pruned_uk.prune.out",
#" --make-bed --out /well/todd/users/jinshaw/aad/under_7/forpcad_uk"))
#system(paste0("~/software/plink2 --bfile /well/todd/users/jinshaw/aad/under_7/forpcad_uk ",
#"--exclude range /well/todd/users/jinshaw/aad/under_7/mhc.txt --make-bed --out /well/todd/users/jinshaw/aad/under_7/forpcad_uk_nomhc"))
#system(paste0("plink --bfile /well/todd/users/jinshaw/aad/under_7/forpcad_uk_nomhc --pca --allow-no-sex --out /well/todd/users/jinshaw/aad/under_7/pcad_uk"))


#read this genotype data in and th pcas, then keep only the hits we're interested in:
g<-read.plink(fam="/well/todd/users/jinshaw/aad/geno_unrel_uk.fam",
bed="/well/todd/users/jinshaw/aad/geno_unrel_uk.bed",
bim="/well/todd/users/jinshaw/aad/geno_unrel_uk.bim")
g<-annot.plink(g)
g@samples$uniqueID<-g@samples$member

pcs<-read.table(file="/well/todd/users/jinshaw/aad/under_7/pcad_uk.eigenvec",as.is=T, header=F)
pcs<-pcs[,c(1:12)]
...
pheno<-cbind(pheno,b)
p1<-pheno
save(b,pheno,t1dsnps, file="/well/todd/users/jinshaw/aad/under_7/pheno_mult_uk.R")
...
write.table(likelihoods, file="/well/todd/users/jinshaw/aad/under_7/results/uk/inds_likelihoods_redo.txt",
col.names=T, row.names=F, quote=F, sep="\t")

likelihoods<-read.table(file="/well/todd/users/jinshaw/aad/under_7/results/uk/inds_likelihoods_redo.txt",
...
png(file="/well/todd/users/jinshaw/output/aad/under_7/multinom/redo/uk/inds_het_tests_all.png", res=400,
...
write.table(likelihoods, file="/well/todd/users/jinshaw/aad/under_7/results/uk/inds_likelihoods_redo_3.txt",
col.names=T, row.names=F, quote=F, sep="\t")

likelihoods<-read.table(file="/well/todd/users/jinshaw/aad/under_7/results/uk/inds_likelihoods_redo_3.txt",
header=T, as.is=T, sep="\t")
rownames(likelihoods)<-likelihoods$altid
likelihoods$loci<-ifelse(likelihoods$loci=="FASLG","TNFSF4",likelihoods$loci)
likelihoods$loci<-ifelse(likelihoods$loci=="BCAR1","CTRB1",likelihoods$loci)
...
geom_vline(aes(xintercept=log10(0.05/56)*-1),colour="red",linetype="dashed") +
geom_vline(aes(xintercept=log10(0.05)*-1),colour="red",linetype="dotted") +
geom_vline(aes(xintercept=2.1), colour="red") +
scale_y_discrete(name="Locus") +
theme(axis.title.y=element_text(size=12), 
axis.text.y=element_text(size=12)) +
scale_x_continuous(name=bquote("-log"[10]~.(paste0("(p) Likelihood ratio test for heterogeneity between <7 and >13"))))

png(file="/well/todd/users/jinshaw/output/aad/under_7/multinom/redo/uk/inds_het_tests_all_inc_midrange.png", res=800,
width=40, height=30, units="cm")
grid.arrange(one,two,ncol=2)
dev.off()
...
png(file="/well/todd/users/jinshaw/output/aad/under_7/multinom/redo/uk/het_tests_midrange_just0.1.png", res=800,
width=40, height=30, units="cm")
grid.arrange(one,two,ncol=2)
dev.off()
...
two<-ggplot(data=r1, aes(logp, as.factor(loci))) + geom_point() +
geom_vline(aes(xintercept=log10(0.05/55)*-1),colour="red",linetype="dashed") +
geom_vline(aes(xintercept=2.1), colour="red", linetype="dotted") +
scale_y_discrete(name="Locus") +
scale_x_continuous(name=bquote("-log"[10]~.(paste0("(p) Likelihood ratio test for heterogeneity between <7 and >13")))) +
coord_cartesian(xlim=c(0,5)) +
theme(axis.title.y=element_text(size=15), 
...
png(file="/well/todd/users/jinshaw/output/aad/under_7/multinom/redo/uk/het_tests_all_inc_midrange_no_nonsig.png", res=800,
width=40, height=30, units="cm")
grid.arrange(one,two,ncol=2)
dev.off()
