...
p<-ts[!is.na(ts$match),]
t1<-table(p$locus)
png(file="/well/todd/users/jinshaw/output/aad/under_7/hla/redo_1/concordance_4digits_3.png",
...
p<-ts[!is.na(ts$match),]
t1<-table(p$locus)
png(file="/well/todd/users/jinshaw/output/aad/under_7/hla/redo_1/concordance_4digits_nodqa1_3.png",
height=20,width=20, units="cm", res=400)
ggplot(data=ts[!is.na(ts$match),], aes(as.factor(locus), fill=as.factor(match))) + geom_bar(position="fill",na.rm =TRUE) +
scale_fill_hue(name="HLA 4 digit\n concordance", breaks=c(0,1),labels=c("FALSE","TRUE")) +
scale_x_discrete(name="Locus") +
...
png(file="/well/todd/users/jinshaw/output/aad/under_7/hla/redo_1/concordance_4digits_nodqa1_3_sm.png",
...
pdf(file="/well/todd/users/jinshaw/output/aad/under_7/hla/redo_1/concordance_4digits_nodqa1_3_sm.pdf",
height=7,width=8)
ggplot(data=ts[!is.na(ts$match),], aes(as.factor(locus), fill=as.factor(match))) + geom_bar(position="fill",na.rm =TRUE) +
scale_fill_hue(name="HLA 4 digit\n concordance", breaks=c(0,1),labels=c("FALSE","TRUE")) +
scale_x_discrete(name="Locus") +
...
#and 2 digit concordance?

doinitialprop2<-function(name1,name2){
dummy<-ifelse((substr(hlaout[,paste0(name1,"1")],1,2)==substr(hlaout[,paste0(name2,"1")],1,2)  & substr(hlaout[,paste0(name1,"2")],1,2)==substr(hlaout[,paste0(name2,"2")],1,2)) |
(substr(hlaout[,paste0(name1,"1")],1,2)==substr(hlaout[,paste0(name2,"2")],1,2)  & substr(hlaout[,paste0(name1,"2")],1,2)==substr(hlaout[,paste0(name2,"1")],1,2)),1,
ifelse(is.na(hlaout[,paste0(name1,"1")]) | is.na(hlaout[,paste0(name1,"2")]) | is.na(hlaout[,paste0(name2,"1")]) | is.na(hlaout[,paste0(name2,"2")]),NA,0))
t1<-data.frame(locus=name1,match=dummy)
return(t1)
}
ts<-mapply(doinitialprop2, name1=c("A","B","DRB1", "DQB1"),
name2=c("HLAA_4digit_","HLAB_4digit_","DRB1_4digit_", "DQB1_4digit_"),SIMPLIFY=F)
ts<-do.call("rbind",ts)
...
png(file="/well/todd/users/jinshaw/output/aad/under_7/hla/redo_1/concordance_2digits_nodqa1_3.png",
height=20,width=20, units="cm", res=400)
ggplot(data=ts[!is.na(ts$match),], aes(as.factor(locus), fill=as.factor(match))) + geom_bar(position="fill",na.rm =TRUE) +
scale_fill_hue(name="HLA 4 digit\n concordance", breaks=c(0,1),labels=c("FALSE","TRUE")) +
scale_x_discrete(name="Locus") +
